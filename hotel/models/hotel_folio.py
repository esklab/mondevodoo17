...
class HotelFolio(models.Model):
    ...
    def action_cancel(self):
        ...
        for rec in self:
            ...
            for product in rec.room_line_ids:
...
class HotelFolioLine(models.Model):
    ...
    def test(self):
        for rec in self:
            room_line_ids = self.env["hotel.folio.line"].search([("product_id","=",rec.product_id.id)])
            if rec.product_id:
                for ligne in room_line_ids:
                    if ligne.checkout_date >= rec.checkin_date:
                        raise ValidationError(_("Cette chambre est déjà occupée"))
