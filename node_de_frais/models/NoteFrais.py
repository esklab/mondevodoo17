...
class NoteFrais(models.Model):
    ...
    duree_field = fields.Integer(string='Duree')
    zone_field = fields.Selection(ZONE_EMP, string='Pays/Region')
    ...
    def create(self, vals):
        ...
        if new_frais.category == "communication":
            montant = self.montant_communication(new_frais.comm_categorie, new_frais.duree_field)
            ...
            montant = self.montant_deplacement_locaux(new_frais.zone_field, new_frais.duree_field)
            new_frais.montant_seuil=montant
            _logger.error("montant = %s",montant)
            _logger.error("montant = %s",new_frais.montant_seuil)
            montant=new_frais.montant_seuil
            ...
            _logger.info("indemnite_zone = %s", new_frais.zone_field)
            _logger.info("indemnite_duree = %s", new_frais.duree_field)
            _logger.info("categorie_emp_all = %s", new_frais.category_emp_all)
            montant = self.montant_indemnite(new_frais.zone_field, new_frais.category_emp_all)
            montant=montant*new_frais.duree_field
            new_frais.montant_seuil=montant
            _logger.error("montant = %s", montant)
    ...
    @api.onchange('duree_field', 'zone_field', 'liaison_zone', 'comm_categorie')
    def compute_montant_seuil(self):
        for record in self:
            if record.category == 'indemnite':
                montant = record.montant_indemnite(record.zone_field, record.category_emp_all) * record.duree_field
                record.montant_seuil = montant
            elif record.category == 'deplacement':
                montant = record.montant_deplacement_locaux(record.zone_field, record.duree_field)
                record.montant_seuil = montant
            elif record.category == 'liaison':
                montant = record.montant_liaison(record.liaison_zone)
                record.montant_seuil = montant
            elif record.category == 'communication' :
                montant = record.montant_communication(record.comm_categorie, record.duree_field)
                record.montant_seuil = montant
            else:
