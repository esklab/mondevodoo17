...
class TeamsCalendarEvent(models.Model):
    ...
    def create(self, values):
        ...
        for vals in values:
            if vals.get('description') and 'Microsoft Teams' in vals.get('description'):
                user_id = vals.get('user_id')
                user = self.env['res.users'].sudo().browse(user_id)
                active_user = user
                vals['teams_link_check'] = True
            if all([vals.get('teams_link_check'), vals.get('recurrency')]):
                raise UserError(
                    "Microsoft Teams Event can't create with Recurring Events..!")
                ...
                if vals.get('allday'):
                    ...
                    end_time = vals['stop']
                meeting_link = self.generate_meeting(vals, attendee,
                                                     active_user)
    ...
    def write(self, values):
        ...
        for record in self:
            ...
            is_meeting = any([record.teams_link_check, values.get(
                'teams_link_check')])
            event_recurrency = any(
                [record.recurrency, values.get('recurrency')])
            if all([is_meeting, event_recurrency]):
                raise UserError(
                    "Microsoft Teams Event can't create with Recurring Events..!")
            elif values.get(
                    'teams_link_check') and not record.teams_meeting_url:
                if not active_user.is_authenticated:
                    raise ValidationError(
                        "Generate an access token to create a Microsoft Teams meeting.")
                ...
                for vals in record.read([]):
                    ...
                    values['teams_meeting_url'] = meeting_link.get(
                        'meeting_url')
                ...
                if not active_user.is_authenticated:
                    raise ValidationError(
                        "Generate an access token to delete a Microsoft Teams meeting.")
                ...
                if not active_user.is_authenticated:
                    raise ValidationError(
                        "Generate an access token to update a Microsoft Teams meeting.")
    ...
    def generate_meeting(self, values, attendees, user):
        ...
        if user.is_authenticated:
            if user.token_expire:
                ...
                raise ValidationError(
                    "Generate new token for Microsoft teams meeting.")
    ...
    def update_event(self, values, user):
        ...
        update_url = "https://graph.microsoft.com/" + \
                     "v1.0/users/{0}/events/{1}".format(user.email,
                                                        self.teams_eventid)
        ...
        if not any([values.get('allday'), rec_allday]):
            ...
            start_date = datetime.strptime(str(values['start_date']),
                                           '%Y-%m-%d') if 'start_date' in values.keys() else False
            end_date = datetime.strptime(str(values['stop_date']),
                                         '%Y-%m-%d') if 'stop_date' in values.keys() else False
