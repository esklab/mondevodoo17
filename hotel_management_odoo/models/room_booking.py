...
class RoomBooking(models.Model):
    ...
    need_service = fields.Boolean(default=True, string="Need Service",
                                  help="Check if a Service to be added with"
                                       " the Booking")
    need_fleet = fields.Boolean(default=True, string="Need Vehicle",
    ...
    need_food = fields.Boolean(default=True, string="Need Food",
                               help="Check if a Food to be added with"
                                    " the Booking")
    need_event = fields.Boolean(default=True, string="Need Event",
    ...
    def action_invoice(self):
        ...
        if booking_list:
            ...
            # Générer les lignes de facturation normales
            ...
            # Ajouter les lignes de note (check-in, check-out, durée)
            for room_line in self.room_line_ids:
                note_content = (
                    f"Date d'arrivée : {room_line.checkin_date.strftime('%d/%m/%Y') if room_line.checkin_date else 'N/A'}\n"
                    f"Date de départ : {room_line.checkout_date.strftime('%d/%m/%Y') if room_line.checkout_date else 'N/A'}\n"
                    f"Durée : {room_line.uom_qty} jour(s)"
                )
                account_move.invoice_line_ids.create([{
                    'name': note_content,
                    'quantity': 0,
                    ...
                    'move_id': account_move.id,
                    'price_subtotal': 0,
                    'display_type': 'line_note',  # Ceci indique qu'il s'agit d'une ligne de note
                }])
        ...
        if tz_name:
            ...
            for pending in pending_event:
                ...
                if pending.date_end.date() == fields.date.today():
                    today_events += 1
            food_items = self.env['product.product'].search_count([('is_food', '=', True)])
