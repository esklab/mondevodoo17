    ...
    def track_shipment(self, **post):
        if post.get('tracking_number'):
            ...
            if freight:
                ...
                return request.render('freight.portal_shipment_not_found')
        else:
            return request.render('freight.portal_shipment_not_found')
...
class BookingsCustom(http.Controller):
    ...
    def portal_my_bookings_create(self, redirect=None, **post):
        ...
        move_type = request.env['freight.move.type'].sudo().search([])
        ...
        fleet_truckers = request.env['fleet.vehicle'].sudo().search([])
...
#                final_dict['trucker'] = truckers.browse(int(post.get('trucker'))).id
                final_dict['trucker_id'] = truckers.browse(int(post.get('trucker'))).id
            #General Data
            if post.get('barcode'):
...
class Dashboard(http.Controller):
    ...
    def get_tiles_data(self):
        emp_data = request.env['freight.dashboard'].get_employee_info()
        return emp_data
